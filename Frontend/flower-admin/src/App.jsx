...
const DashboardView = ({ onLogout }) => {
  ...
  const updateStatus = async (orderId, status) => {
    if (
      !confirm(
        `คุณแน่ใจหรือไม่ว่าต้องการ ${status === "approved" ? "ยอมรับ" : "ปฏิเสธ"} ออเดอร์นี้`,
      )
    )
      return;
    try {
      ...
      <main className="p-8 max-w-7xl mx-auto">
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={fetchOrders}
              className="p-3 bg-white rounded-xl border border-gray-100 hover:bg-gray-50"
            >
              <RefreshCw
                size={20}
                className={
                  ...
                  <tr key={order.orderId} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-6 py-4 text-center">
                      {order.status === "pending" ? (
                        ...
                        <span
                          className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${order.status === "approved" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
                        >
                          {order.status}
                        </span>
                      )}
        ...
        {isModalOpen && selectedOrder && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#FDFBF7] rounded-[2.5rem] shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto transform animate-in zoom-in-95">
              <button
                onClick={() => setIsModalOpen(false)}
                className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-md z-10 text-gray-400 hover:text-red-500 transition-colors"
              >
                <X size={20} />
              </button>
              ...
              <div className="p-8 md:p-10">
                <div className="text-center mb-8">
                  <div
                    className={`mx-auto mb-4 w-16 h-16 rounded-full flex items-center justify-center ${selectedOrder.status === "approved" ? "bg-green-100 text-green-500" : "bg-orange-100 text-orange-500"}`}
                  >
                  ...
                  <h2 className="text-2xl font-serif text-[#5D6D4E] font-bold">
                    ข้อมูลคำสั่งซื้อ
                  </h2>
                  <p className="text-gray-400 text-sm">
                    สถานะปัจจุบัน: {selectedOrder.status}
                  </p>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl mb-6 shadow-sm border border-[#F0EAD6]">
                  ...
                  <div className="grid grid-cols-1 gap-3 text-xs text-gray-500">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className="leading-tight">
                        เบอร์ติดต่อ: {selectedOrder.customerInfo?.phone}
                      </span>
                    </div>
                    <div className="flex items-start gap-2">
                      <Mail size={14} className="text-[#8A9A7B] shrink-0" />
                      <span className="leading-tight">
                        อีเมล์: {selectedOrder.customerInfo?.email}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-8">
                  <h4 className="font-bold text-[10px] uppercase tracking-widest text-[#8A9A7B] border-b pb-2">
                    รายการที่สั่งซื้อ
                  </h4>
                  {selectedOrder.items?.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex gap-4 bg-white p-3 rounded-2xl border border-gray-50"
                    >
                      <div
                        ...
                        onClick={() =>
                          item.snapshot && setZoomFlowerImg(item.snapshot)
                        }
                      >
                        {item.snapshot ? (
                          <img
                          ...
                          <Flower className="m-auto mt-4 text-gray-200" />
                        )}
                      </div>
                        ...
                        {item.type === "custom" && (
                          <div className="mt-2 flex flex-wrap gap-2">
                            {groupFlowers(item.details).map((g, i) => (
                              <span
                                key={i}
                                className="flex items-center gap-2 text-[10px] bg-[#F8F9F4] pl-1.5 pr-2.5 py-1 rounded-full text-[#5D6D4E] font-bold border border-[#F0EAD6]"
                              >
                                {/* จุดสีวงกลม */}
                                <div
                ...
                <div className="bg-[#5D6D4E] text-white p-6 rounded-3xl mb-8 flex justify-between items-center shadow-lg">
                  <div>
                    <span className="text-[10px] uppercase tracking-widest opacity-70 block">
                      ยอดรวมสุทธิ
                    </span>
                  ...
                  <span className="text-3xl font-bold">
                    ฿{selectedOrder.summary?.totalPrice.toLocaleString()}
                  </span>
                </div>

                <div className="space-y-3">
                  <h4 className="font-bold text-[10px] uppercase tracking-widest text-[#8A9A7B] text-center">
                    หลักฐานการโอนเงิน
                  </h4>
                  <div className="bg-white p-4 rounded-3xl border-2 border-dashed border-gray-200 flex justify-center relative">
                    <img
                      ...
                      onClick={() =>
                        window.open(
                          `http://72.62.243.238:5000/${selectedOrder.slipPath}`,
                        )
                      }
                    />
                  </div>
          ...
          >
            <div className="relative max-w-2xl w-full flex flex-col items-center animate-in zoom-in-95 duration-200">
              ...
              <img
                ...
                alt="Flower Zoom"
              />
              <p className="text-[#5D6D4E] mt-4 font-serif italic text-sm">
                Flower Snapshot Preview
              </p>
            </div>
          </div>
        )}
